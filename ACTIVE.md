...
12. **On-device verification needed for all 2026-08-21 changes** — Growth's new content, Health's new Sport dropdown/scoring, the Faith lag fix, the Onboarding/Profile flow, and the fully-redesigned Home tab visuals (hourglass with solid green sand, the Day ring next to Code Score Today, and the striped glowing Week/Year bars below Calendar) were all only verified via Playwright/desktop, not on Davydenko's actual phone. Push to GitHub Pages (none of this has been pushed yet), then fully close and reopen the PWA (not just background it) to clear the service-worker cache, then spot-check: log a Salat and confirm no lag, try the Squats/Abs dropdowns, confirm the onboarding nickname shows correctly in both the header and the Face ID lock screen, confirm the hourglass sand renders bright green (his real DOB is already on file from onboarding, no need to re-enter it), and confirm the Day ring plus the striped Week/Year bars render and glow correctly.
